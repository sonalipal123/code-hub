...
const JSON_SOURCE = "/json/angularJs_programs.json";

// ---------- Global State ----------
let ALL_ITEMS = []; // All programs from JSON
let USER_BOOKMARKS = []; // IDs from MongoDB
...
function renderList(items, totalCount, isFiltered) {
  ...
  if (!items.length) {
    listArea.innerHTML = `
      <div class="alert alert-light text-center border p-5">
        <h4>${
          SHOWING_BOOKMARKS_ONLY ? "No Bookmarks Yet" : "No Programs Found"
        }</h4>
        <p class="text-muted">${
  ...
  items.forEach((item) => {
    ...
    bmBtn.addEventListener("click", async () => {
      ...
      if (updated) USER_BOOKMARKS = updated.filter((id) => id.startsWith("angular-"));
...
(async function load() {
  try {
    const res = await fetch("/json/angularJs_programs.json");
    if (!res.ok)
      throw new Error("Failed to load angularJs_programs.json (" + res.status + ")");
    ...
    ALL_ITEMS = data.map((item, index) => {
      if (!item.id) {
        ...
        const slug = (item.title || "prog")
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-");
        return { ...item, id: `angular-${slug}-${index}` };
      }
      return item;
    });
    const allBookmarks = await fetchUserBookmarks();
    USER_BOOKMARKS = allBookmarks.filter((id) => id.startsWith("angular-"));
    applyFiltersAndRender();
  } catch (err) {
    listArea.innerHTML =
