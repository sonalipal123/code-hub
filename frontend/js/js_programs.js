...
const JSON_SOURCE = "/json/js_programs.json";
...
function renderList(items, totalCount, isFiltered) {
  ...
  items.forEach((item) => {
    ...
    bmBtn.addEventListener("click", async () => {
      ...
      if (updated) USER_BOOKMARKS = updated.filter((id) => id.startsWith("js-"));
...
(async function load() {
  try {
    ...
    if (!res.ok)
      throw new Error("Failed to load js_programs.json (" + res.status + ")");
    ...
    ALL_ITEMS = data.map((item, index) => {
      if (!item.id) {
        ...
        const slug = (item.title || "prog")
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-");
        return { ...item, id: `js-${slug}-${index}` };
      }
      return item;
    });
    const allBookmarks = await fetchUserBookmarks();
    USER_BOOKMARKS = allBookmarks.filter((id) => id.startsWith("js-"));
    applyFiltersAndRender();
  } catch (err) {
    listArea.innerHTML =
