...
const JSON_SOURCE = "/json/cpp_programs.json";
...
function renderList(items, totalCount, isFiltered) {
  ...
  items.forEach((item) => {
    ...
    bmBtn.addEventListener("click", async () => {
      ...
      if (updated) USER_BOOKMARKS = updated.filter((id) => id.startsWith("cpp-"));
...
(async function load() {
  try {
    ...
    if (!res.ok)
      throw new Error("Failed to load cpp_programs.json (" + res.status + ")");
    ...
    ALL_ITEMS = data.map((item, index) => {
      if (!item.id) {
        ...
        const slug = (item.title || "prog")
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-");
        return { ...item, id: `cpp-${slug}-${index}` };
      }
      return item;
    });
    const allBookmarks = await fetchUserBookmarks();
    USER_BOOKMARKS = allBookmarks.filter((id) => id.startsWith("cpp-"));
    applyFiltersAndRender();
  } catch (err) {
    listArea.innerHTML =
