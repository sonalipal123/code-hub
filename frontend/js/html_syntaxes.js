...
const JSON_SOURCE = "/json/html.json";
...
function renderList(items, totalCount, isFiltered) {
  ...
  items.forEach((item) => {
    ...
    bmBtn.addEventListener("click", async () => {
      ...
      if (updated) USER_BOOKMARKS = updated.filter((id) => id.startsWith("html-"));
...
(async function load() {
  try {
    ...
    if (!res.ok)
      throw new Error("Failed to load html.json (" + res.status + ")");
    ...
    ALL_ITEMS = data.map((item, index) => {
      if (!item.id) {
        ...
        const slug = (item.title || "prog")
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-");
        return { ...item, id: `html-${slug}-${index}` };
      }
      return item;
    });
    const allBookmarks = await fetchUserBookmarks();
    USER_BOOKMARKS = allBookmarks.filter((id) => id.startsWith("html-"));
    applyFiltersAndRender();
  } catch (err) {
    listArea.innerHTML =
